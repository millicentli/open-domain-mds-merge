...
def preprocess_ms2_marginalization(
    ...
    background = text.strip()
    articles = [[f"{t.strip()} {a.strip()}" for t, a in zip(title, abstract)] for title, abstract in zip(titles, abstracts)]
    # Following https://aclanthology.org/2021.emnlp-main.594/, take the first 25 articles.
    articles = [article[:max_included_studies] for article in articles]
    # text = f" {doc_sep_token} ".join([background] + articles)
    texts = [f" {doc_sep_token} ".join([background] + article) for article in articles]
    summary = summary.strip()
