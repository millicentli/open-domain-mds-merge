...
Finetuning and evaluation over multiple parts, which is explained below.

> Training/finetuning <
For training, we train our model on two different options:
1. The original dataset, single split.
2. The modified (re-retrieved) dataset, single split.

We set up this training paradigm so that we can slot in our own models easily.
...
/home/li.mil/open-domain-mds-merge/open-mds/output/datasets/tiny_mslr2022

testing the small subset, reretrieved:
/home/li.mil/open-domain-mds-merge/open-mds/output/datasets/tiny_ms2_retrieved_split=1
"""
