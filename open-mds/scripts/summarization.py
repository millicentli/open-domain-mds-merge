...
Training:
For training on the vanilla dataset:
python ./scripts/finetune_summarization.py "./conf/base.yml" "./conf_updated/ms2/led-base/train_original.yml" \
        output_dir="./output/ms2/led-base/original" \
...
####
python ./scripts/summarization.py "./conf_updated/base.yml" "./conf_updated/cochrane/led-base/train_retrieved.yml" \
        output_dir="/scratch/li.mil/open-domain-mds-merge/models/cochrane/led-base/retrieved" \
        dataset_name="/work/frink/li.mil/data/marginalization-datasets/cochrane_retrieved_split=2"

python ./scripts/summarization.py "./conf_updated/base.yml" "./conf_updated/cochrane/led-base/train_noisy_retrieved.yml" \
        output_dir="/scratch/li.mil/open-domain-mds-merge/models/cochrane/led-base/retrieved"

python ./scripts/summarization.py "./conf_updated/base.yml" "./conf_updated/cochrane/led-base/train.yml" \
        output_dir="/scratch/li.mil/open-domain-mds-merge/models/cochrane/led-base/vanilla"
...
def main():
    ...
    if "llama" not in model_args.model_name_or_path:
        if (
        ...
        for i in range(len(examples[text_column])):
            ...
            if examples[text_column][i]:
                if data_args.dataset_name == "multi_news" or "multinews" in data_args.dataset_name:
                    ...
                    # If it's the retrieved dataset, then we need to turn each subset into its own data point to train on
                    if "retrieved" in data_args.dataset_name:
                        text, summary = util.preprocess_cochrane_marginalization(
                            summary=examples[summary_column][i],
                            titles=examples["title"][i],
                ...
                if isinstance(text, list):
                    text = [util.sanitize_text(t) for t in text]

                    inputs.extend(text)
                    targets.extend([summary] * len(text))
                else:
